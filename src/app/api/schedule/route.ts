...
const Body = z.object({
  ...
  postHourLocal: z.number().int().min(0).max(23).optional(),
});

export async function POST(req: NextRequest) {
  try {
    const { tweets, socialAccountIds, startDate, timezone, postHourLocal } = Body.parse(
      await req.json()
    );

    const env = getEnv();
    const tz = timezone || env.DEFAULT_TIMEZONE;
    const hour = (postHourLocal ?? parseInt(env.DEFAULT_POST_HOUR_LOCAL, 10)) || 21;
    ...
    let first = start.set({ hour, minute: 0, second: 0, millisecond: 0 });
